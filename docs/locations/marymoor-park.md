# Location & Event Plan — Marymoor Park (Redmond, WA)

**Location type:** Regional county park (informal practice hunt)
**Status:** PRIMARY — August informal practice hunt
  (Sat Aug 22, 2026, pending site walk)
**Last updated:** 2026-07-19

---

## 1. Event summary

| Field            | Value                                       |
| ---------------- | ------------------------------------------- |
| Event type       | ARDF informal practice hunt                 |
| Proposed date    | Sat Aug 22, 2026 (PRIMARY — see §9)          |
| Date notes       | Evening concert that day — see §9            |
| Proposed time    | 12:00 PM – 2:30 PM (2.5 hours)              |
| Format           | Single fox (1-fox) on-foot hunt             |
| Host club        | Mike & Key ARC — Fox Hunting Program        |
| Park             | Marymoor Park, 6046 W Lake Sammamish Pkwy   |
|                  | NE, Redmond, WA 98052                       |

---

## 2. Why Marymoor

- ~640 acres — the region's largest county park, with varied terrain
  (open fields, wooded river corridor, lake frontage).
- Large parking lots, plus light-rail access via the 2 Line (Downtown
  Redmond / Marymoor Village) with connecting buses.
- Mix of open and wooded ground gives good direction-finding variety
  for new and experienced hounds.

---

## 3. Park rules & permit requirements

Source: King County Parks (kingcounty.gov), 2026.

### Do we need a permit or reservation?

For a small, **informal practice hunt**, **most likely no** — provided we
stay under the thresholds below and do not publicly advertise it as a
formal event.

- **Shelter reservation:** required only for groups of **35 or more**
  (KCC 7.12.030). Reserve up to 365 days out, minimum 7 days ahead.
- **Special Event Permit:** required for groups **over 50**, OR for any
  size involving vendors, tents/structures, ticket sales, amplified
  sound, or public advertising.
- If we expect 35+ people or want a guaranteed base shelter, contact
  the scheduling office (see Section 7).

### Operating rules to plan around

- **Hours:** open daily 8:00 AM – dusk. Our window is well within this.
- **Parking:** small daily fee (~$1–3) on busy days; summer Sunday lots
  fill early — arrive ahead of the start.
- **Off-leash dog area (42 acres, east side):** avoid for both start and
  beacon. Loose dogs and heavy traffic; dogs may disturb the
  transmitter.
- **Amplified-sound permit** covers PA systems / music — NOT our radios.
  Low-power ARDF transmitters operate under our amateur license. All
  operators must be licensed; coordinate the frequency in advance.
- **No drones / RC aircraft** without a separate permit.
- **General:** leash dogs outside the OLA, pack out all trash, no alcohol
  without a permit, no signs nailed to trees, stay out of Clise Gardens.

---

## 4. Start point & base

**Candidate:** A central picnic shelter / lawn near the athletic fields
and Clise Mansion area.

- Obvious landmark with nearby parking and restrooms.
- Gives hounds a clean radial spread in multiple directions.
- Use for the safety briefing, gear check, frequency check, and timing.

---

## 5. Beacon (fox) placement

**Candidate:** Wooded natural area on the northeast side — the Audubon
Bird Loop / Sammamish River corridor.

- Real tree canopy provides a genuine RDF challenge.
- Walkable on-trail or just off-trail; ~0.5–0.8 mi from the start for
  meaningful bearings.
- Contained safely within the park boundary.

**Avoid:**

- The off-leash dog area (east side).
- The velodrome and concert lawn (crowds + possible RF noise).
- Clise Gardens and any private/planted beds.

**Geometry tip:** Place the start west/central and the beacon to the
northeast so initial bearings cross the open fields — easy to teach,
with a tougher wooded approach near the end.

> NOTE: All start/beacon locations are planning candidates only.
> Confirm access, tree cover, and current conditions during a site walk
> before the event.

---

## 6. Transmitter / frequency plan

Based on current program equipment status (see
`docs/reports/2026-06-07-fox-transmitter-status.md`).

| Field         | Value                                          |
| ------------- | ---------------------------------------------- |
| Fox unit      | Byonics MicroFox (MF-15 or MF-PC), 1 deployed  |
| Callsign      | K7LED                                          |
| TX frequency  | 147.42 MHz (confirm for this hunt)             |
| Cycle         | 15 s on / 15 s off                             |
| CW ID         | K7LED at 20 WPM                                 |

> Deploy only ONE fox until units are reprogrammed for a multi-fox
> format (both are currently programmed identically).

---

## 7. Logistics & contacts

- **Marymoor scheduling office:** marymoorrentals@kingcounty.gov /
  206-477-7275 (for shelter reservations or permit questions).
- **Parking:** budget for the daily fee; carpool encouraged.
- **Transit:** 2 Line light rail (Downtown Redmond / Marymoor Village)
  plus connecting bus for car-free hounds.
- **Restrooms:** available near central shelters/fields.

---

## 8. Open items / to-do

- [ ] Site walk to confirm start point and beacon hide.
- [ ] Confirm head-count; reserve a shelter if 35+ attendees.
- [ ] Confirm practice frequency (147.42 MHz or alternate).
- [ ] Decide 1-fox vs. multi-fox format.
- [ ] Recruit a fox hider / mentor for new participants.
- [ ] Verify summer dusk time and lot opening for early setup.
- [ ] Confirm date Sat Aug 22, 2026 with the club.
- [ ] Note: concert (gates 4 PM) — arrive early, wrap by 2:30.

---

## 9. Scheduling notes & related plans

### Day-of note — Sat Aug 22, 2026

- **Marymoor Live concert** that evening: Alison Krauss & Union Station
  feat. Jerry Douglas — **show 6:00–8:00 PM, gates 4:00 PM.** Does not
  overlap the noon–2:30 hunt (~1.5 hr buffer before gates).
- **Watch-outs late in the window (≈1:30–2:30):** concert sound checks
  (amplified audio from the amphitheater) and parking lots starting to
  fill as staff/vendors/early fans arrive.
- **Mitigation:** arrive early, wrap by 2:30, and prefer the east/central
  lots near the athletic fields over the amphitheater lots. The
  amphitheater is on the west side, opposite the NE beacon and central
  start — the course itself is clear.

### Related plans

- `docs/locations/flaming-geyser-state-park.md` — July practice hunt.
- `docs/locations/howarth-park.md` — September regional / mobile hunt.
